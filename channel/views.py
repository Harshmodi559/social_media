...
# from pyrsistent import l
from .models import Post, Profile, FollowersCount, LikePost, Comment
...
def index(request):
   ...
   # now to find all user_sugg_list profiles
    ...
    return render(request, 'index.html', {'user_profile': user_profile, 'posts': feed_list, 'suggestions_username_profile_list': suggestions_username_profile_list, 'comments': allcomments})
...
def comments(request, id):
    ...
    if(request.method == "POST"):
        ...
        newCommmentCreate = Comment.objects.create(
            post_idd=post_id, body=body, user=current_user, profile_image=user_profile_image, comment_post_id=post.id)
        newCommmentCreate.save()
...
def like_post(request, id):
    ...
    already_liked = LikePost.objects.filter(
        post_id=post_id, username=post_like_user).first()
#
    if(already_liked != None):
        ...
        new_user_liked = LikePost.objects.create(
            post_id=post_id, username=post_like_user, owner=post.user)
        new_user_liked.save()
        post.no_of_likes = post.no_of_likes+1
        post.save()
